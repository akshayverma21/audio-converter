...
#!/usr/bin/env bash
set -e # Exit on error; 'x' removed to reduce verbosity unless debugging is needed
...
python manage.py migrate --noinput || { echo "Migrations failed! Check logs for details."; exit 1; }

# Create superuser if env vars present
if [ -n "$DJANGO_SUPERUSER_EMAIL" ] && [ -n "$DJANGO_SUPERUSER_PASSWORD" ]; then
  echo "Ensuring superuser exists..."
  python manage.py createsuperuser --noinput --username admin --email "$DJANGO_SUPERUSER_EMAIL" --password "$DJANGO_SUPERUSER_PASSWORD" || {
    echo "Superuser creation failed or user already exists.";
  }
fi
